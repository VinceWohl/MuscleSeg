...
def get_files(main_folder):
    """Find Fat images, whole-muscle masks, and compartment masks at any folder depth."""
    FatImgs = []
    WholeMasks = []
    ThighCompMasks = []
    CalfCompMasks = []
    ...
    for dirpath, dirnames, filenames in sorted(os.walk(main_folder)):
        ...
        if 'Fat.nii.gz' in filenames and 'WHOLEMUSCLE_SAT_mask.nii.gz' in filenames:
            FatImgs.append(os.path.join(dirpath, 'Fat.nii.gz'))
            WholeMasks.append(os.path.join(dirpath, 'WHOLEMUSCLE_SAT_mask.nii.gz'))

        if 'MUSCLECOMP_mask.nii.gz' in filenames:
            if folder_name == 'thigh':
                ThighCompMasks.append(os.path.join(dirpath, 'MUSCLECOMP_mask.nii.gz'))
            elif folder_name == 'calf':
                CalfCompMasks.append(os.path.join(dirpath, 'MUSCLECOMP_mask.nii.gz'))
...
def INTERSECTION(main_folder, app_folder, QUANTIFY_IMAT):
    ...
    FatImgs, WholeMasks, ThighCompMasks, CalfCompMasks = get_files(main_folder)
    ...
    for idx in range(len(FatImgs)):
        ...
        if QUANTIFY_IMAT and 'thigh' in FatImgs[idx].lower() and len(ThighCompMasks)>0:
            '''
            ##################################################################################################################
        ...
        elif QUANTIFY_IMAT and 'calf' in FatImgs[idx].lower() and len(CalfCompMasks)>0:
            '''
            ##################################################################################################################
    ...
    file_path = os.path.join(app_folder, "MAT_volumes.xlsx")
