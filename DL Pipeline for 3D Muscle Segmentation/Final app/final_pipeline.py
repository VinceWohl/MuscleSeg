...
import time
import torch
from final_dataset import MRI_Dataset
from final_model_A import UNET as MODEL_A
from final_model_B import UNET as MODEL_B
from final_model_C import UNET as MODEL_C
from final_bridge import BRIDGE
from final_intersection import INTERSECTION
...
def APPLY_PIPELINE(SOURCE_FOLDER,
    ...
    if APPLY_MODEL_A:
        ...
        checkpoint = torch.load(os.path.join(APP_FOLDER, CHECKPOINT_A), map_location=torch.device(DEVICE))
    ...
    if APPLY_MODEL_B:
        ...
        checkpoint = torch.load(os.path.join(APP_FOLDER, CHECKPOINT_B), map_location=torch.device(DEVICE))
        model.load_state_dict(checkpoint['model_state_dict'])
        print('=> Apply Model B')
    ...
    if APPLY_MODEL_C:
        ...
        checkpoint = torch.load(os.path.join(APP_FOLDER, CHECKPOINT_C), map_location=torch.device(DEVICE))
        model.load_state_dict(checkpoint['model_state_dict'])
        print('=> Apply Model C')
