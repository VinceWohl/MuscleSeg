...
This script crops MR images around the whole muscle region using Model A output as reference.
"""
...
def get_files(main_folder):
    """Find Fat, In_phase, Opp_phase, Water and WHOLEMUSCLE_SAT_mask files.

    Walks the directory tree to find scan folders (any depth) that contain
    a WHOLEMUSCLE_SAT_mask.nii.gz (produced by Model A).
    """
    FirCImgs = []
    SecCImgs = []
    ThiCImgs = []
    ...
    for dirpath, dirnames, filenames in sorted(os.walk(main_folder)):
        dirnames.sort()
        if 'WHOLEMUSCLE_SAT_mask.nii.gz' in filenames:
            FirCImgs.append(os.path.join(dirpath, 'Fat.nii.gz'))
            SecCImgs.append(os.path.join(dirpath, 'In_phase.nii.gz'))
            ThiCImgs.append(os.path.join(dirpath, 'Opp_phase.nii.gz'))
            FouCImgs.append(os.path.join(dirpath, 'Water.nii.gz'))
            WholeMasks.append(os.path.join(dirpath, 'WHOLEMUSCLE_SAT_mask.nii.gz'))
...
def BRIDGE(main_folder):
    ...
    for idx in range(len(WholeMasks)):
        ...
        # save the cropped files with a 'cropped_' prefix
        folder = os.path.dirname(FirCImgs[idx])
        fat_name = os.path.join(folder, 'cropped_Fat.nii.gz')
        in_name = os.path.join(folder, 'cropped_In_phase.nii.gz')
        opp_name = os.path.join(folder, 'cropped_Opp_phase.nii.gz')
        water_name = os.path.join(folder, 'cropped_Water.nii.gz')
    ...
    print('=> saved cropped versions of Fat, Water, In_phase, Opp_phase scans')
