...
@author: Vincent Wohlfarth

This script provides utility functions for file discovery, preprocessing, and model application.
"""
...
def get_files_A(main_folder):
    """Find Water and Fat files in Calf/Thigh folders at any depth."""
    FirCImgs, SecCImgs = [], []
    for dirpath, dirnames, filenames in sorted(os.walk(main_folder)):
        dirnames.sort()
        folder_name = os.path.basename(dirpath).lower()
        if folder_name in ('calf', 'thigh') and 'Water.nii.gz' in filenames and 'Fat.nii.gz' in filenames:
            FirCImgs.append(os.path.join(dirpath, 'Water.nii.gz'))    # 1st channel = Water
            SecCImgs.append(os.path.join(dirpath, 'Fat.nii.gz'))      # 2nd channel = Fat
    return [FirCImgs, SecCImgs]


def get_files_B(main_folder):
    """Find cropped Opp_phase and Water files in Thigh folders (bridge output)."""
    FirCImgs = []
    SecCImgs = []
    ...
    for dirpath, dirnames, filenames in sorted(os.walk(main_folder)):
        dirnames.sort()
        folder_name = os.path.basename(dirpath).lower()
        if folder_name == 'thigh' and 'cropped_Opp_phase.nii.gz' in filenames:
            FirCImgs.append(os.path.join(dirpath, 'cropped_Opp_phase.nii.gz'))
            SecCImgs.append(os.path.join(dirpath, 'cropped_Water.nii.gz'))

    return [FirCImgs, SecCImgs]


def get_files_C(main_folder):
    """Find cropped Opp_phase and Water files in Calf folders (bridge output)."""
    FirCImgs = []
    SecCImgs = []
    ...
    for dirpath, dirnames, filenames in sorted(os.walk(main_folder)):
        dirnames.sort()
        folder_name = os.path.basename(dirpath).lower()
        if folder_name == 'calf' and 'cropped_Opp_phase.nii.gz' in filenames:
            FirCImgs.append(os.path.join(dirpath, 'cropped_Opp_phase.nii.gz'))
            SecCImgs.append(os.path.join(dirpath, 'cropped_Water.nii.gz'))
...
class CropOrPadXY(tio.Transform):
    ...
    def apply_transform(self, subject):
        ...
        subject['image'] = tio.ScalarImage(tensor=img)
...
def model_application(model, dic, app_ds, patch_size, app_folder, mID, shape_diffs=None, device='cuda'):
    ...
    # create a classwise matrix to store the class volumes for every sample
    cw_matrix = []
    head_row = ['Image']
    ...
    for idx in range(len(app_ds)):
        ...
        # retransform to initial size
        pred_array = retransform(pred_array, app_ds.get_init_WnH(idx))

        # restore original image shape and manage filenames
        scan_folder = os.path.dirname(app_ds.FirCImgs[idx])
        if mID == 'A':
            pred_dir = os.path.join(scan_folder, 'WHOLEMUSCLE_SAT_mask.nii.gz')
            excel_dir = os.path.join(app_folder, 'WHOLEMUSCLE_SAT_volumes.xlsx')
        elif mID == 'B':
            pred_dir = os.path.join(scan_folder, 'MUSCLECOMP_mask.nii.gz')
            excel_dir = os.path.join(app_folder, 'THIGH_MUSCLECOMP_volumes.xlsx')
            ...
            pred_dir = os.path.join(scan_folder, 'MUSCLECOMP_mask.nii.gz')
            excel_dir = os.path.join(app_folder, 'CALF_MUSCLECOMP_volumes.xlsx')
