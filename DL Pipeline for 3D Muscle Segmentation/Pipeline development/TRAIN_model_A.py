...
This scripts trains a model to segment whole muscle and SAT in thigh and calf MR images.
"""
...
from transform_fns import get_transforms_A
from dataset import MRI_Dataset
from model_A import UNET
from loss_fns import (BCEDiceLoss, DiceScore)
...
LEARNING_RATE = 1e-4
NUM_WORKERS = 2
IMAGE_WIDTH = 368   # original 384
IMAGE_HEIGHT = 256  # original 288
IMAGE_DEPTH = 32    # original 45-68
PIN_MEMORY = True
TEST_MODEL = True
...
def main():
    for fold in range(1,NUM_FOLDS+1):
        print(f'--------------------------------------- MODEL_Av{fold} STARTS ---------------------------------------')
        start_time = time.time()
        model_path = f'Model_A_exp_{EXP}/v{fold}'
        os.makedirs(model_path, exist_ok=True)
        
        # get data directions and partition them
        IMGS, MASKS = get_files_A(DATA_PATH)
        TRAIN_IMGS, TRAIN_MASKS, VAL_IMGS, VAL_MASKS, TEST_IMGS, TEST_MASKS = splitup(IMGS, MASKS, model_path, fold)
                
        # get transform functions
        TRAIN_TF, VAL_TF, TEST_TF = get_transforms_A(IMAGE_WIDTH, IMAGE_HEIGHT, IMAGE_DEPTH)
        ...
        loss_fn = BCEDiceLoss(num_classes=NUM_CLASSES)
        ...
        print('------------------------------------------------------------------------')
        print(f'MODEL_Av{fold}: Training')
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            if v_loss < best_val[0]:
                ...
                checkpoint_path = f"{check_path}/cp_model_Av{fold}_epoch_{epoch}.pth"
        ...
        print(f'Runtime for training Model Av{fold}: {hours}:{minutes}')
        print('------------------------------------------------------------------------')
        
        if TEST_MODEL:
            print(f'MODEL_Av{fold}: Testing')
            test_folder = f'{model_path}/test_results'
            os.makedirs(test_folder,exist_ok=True)
            
            # load the model with the best validation result
            checkpoint = torch.load(f'{check_path}/cp_model_Av{fold}_epoch_{best_val[1]}.pth', map_location=torch.device(DEVICE))
            model.load_state_dict(checkpoint['model_state_dict'])
            print(f'=> Best checkpoint loaded: cp_model_Av{fold}_epoch_{best_val[1]}.pth')
            
            # apply model for testing
            model_application(model, DIC, test_ds, test_fn, IMAGE_DEPTH, test_folder, mID='A', device=DEVICE)
       
        print(f'--------------------------------------- MODEL_Av{fold} COMPLETE ---------------------------------------')     
